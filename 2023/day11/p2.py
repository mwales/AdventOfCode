...
class Galaxy:
	...
	def createExpandedGalaxy(self, expFactor):
		expFactor -= 1    # I have no idea why i have to do this
		listOfColsToExpand = []
		listOfRowsToExpand = []
		...
		ng.initGalaxySize(self.width + len(listOfColsToExpand) * expFactor, self.height + len(listOfRowsToExpand) * expFactor)
		for p in self.galaxies:
			modX = p[0] + numInListLessThan(listOfColsToExpand, p[0]) * expFactor
			modY = p[1] + numInListLessThan(listOfRowsToExpand, p[1]) * expFactor
...
def main(argv):
	...
	eg = g.createExpandedGalaxy(1000000)
	#eg.printGalaxy()
