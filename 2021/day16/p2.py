...
class BITS:
	...
	def evaluate(self):
		...
		if self.type == 0:
			...
			for sp in self.subPackets:
				retVal += sp.evaluate()
			eprint("evaluate(sum) = {}".format(retVal))
			return retVal
		...
		if self.type == 1:
			...
			for sp in self.subPackets:
				retVal *= sp.evaluate()
			eprint("evaluate(product) = {}".format(retVal))
			return retVal
			
		if self.type == 2:
			# minimum
			retVal = min( [ sp.evaluate() for sp in self.subPackets ] )
			eprint("evaluate(min) = {}".format(retVal))
			return retVal
		
		if self.type == 3:
			# maximum
			retVal = max( [ sp.evaluate() for sp in self.subPackets ] )
			eprint("evaluate(max) = {}".format(retVal))
			return retVal
		
		# everything left is comparison operations
		leftSide = self.subPackets[0].evaluate()
		rightSide = self.subPackets[1].evaluate()
...
def main(argv):
	...
	print("Evaultion: {}".format(b.evaluate()))
