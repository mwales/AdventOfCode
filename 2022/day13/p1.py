...
def main(argv):
	...
	for i in range(0, len(data), 3):
		...
		left = data[i]
		right = data[i+1]
		eprint("IDX {}, left= {}, right= {}".format(curIdx, left, right))

		lds = DistressSignal(left)
		rds = DistressSignal(right)

		if (lds < rds):
			eprint("Pair {} is in order".format(curIdx))
			sumIdx += curIdx
		else:
			eprint("Pair {} is not in order".format(curIdx))

		eprint("=====\n\n")
	

	print("Sum Of all in order idx = {}".format(sumIdx))
