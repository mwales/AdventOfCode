...
class Monkey:
	...
	def meetOtherMonkeys(self, monkeyList, mf):
...
def main(argv):
	...
	masterTestSet = set()
	for m in monkeyList:
		masterTestSet.add(m.divisibleTest)

	masterFactor = 1
	for v in masterTestSet:
		masterFactor *= v

	print("Master Factor = {}".format(masterFactor))


	eprint("List of monkeys:")
	for m in monkeyList:
		eprint(m.dump(True))
		m.meetOtherMonkeys(monkeyList, masterFactor)
	...
	print("Num investigations: {}".format(inspectionTimes))
	inspectionTimes.sort(reverse=True)
	print("Num investigations: {}".format(inspectionTimes))

	solve1 = inspectionTimes[0] * inspectionTimes[1]

	print("Solve 2 = {}".format(solve1))

	# Wrong answer 2713310158
