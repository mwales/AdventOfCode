...
def listAllDirSizes(dirData):
	retVal = []
	retVal.append(computeSizeOfDir(dirData))
	
	for curName in dirData.keys():
		eprint("  sumOfValidDirSums name = {} and dc = {}".format(curName, dirData[curName]))
		if type(dirData[curName]) == type({}):
			retVal.extend(listAllDirSizes(dirData[curName]))
...
def main(argv):
	...
	rootSize = computeSizeOfDir(root_dir)
	eprint("Full size = {}".format(rootSize))

	fsSize    = 70000000
	fsFreeReq = 30000000
	if rootSize > fsSize:
		eprint("Nothing fits on disk")
	else:
		eprint("root fits on disk")
		minDeleteSize = rootSize - (fsSize - fsFreeReq) 
	eprint("Min delete size = {}".format(minDeleteSize))

	allDirs = listAllDirSizes(root_dir)
	allDirs.sort()
	eprint("All dirs = {}".format(allDirs))

	for ds in allDirs:
		if (ds > minDeleteSize):
			print("Min delete size = {}".format(ds))
			break
